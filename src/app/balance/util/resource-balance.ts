...
export class ResourceBalance {
    ...
    setMessageValidacion (
        ...
        if (!balanced) {
          this.createMessage('alert-danger', 'Error. Inconsistencia en los saldos.', messageValidacion);
          this.render.setStyle(btnAutorizar.nativeElement, 'display', 'none');
          return 0;
        }
    ...
    validationChangeIcons (
        ...
        let iconOk = '<svg #iconVerifyRCV width="1.3em" height="1.3em" viewBox="0 0 16 16" class="bi bi-check-circle icon-verify-ok" fill="currentColor" xmlns="http://www.w3.org/2000/svg"> <path fill-rule="evenodd" d="M8 15A7 7 0 1 0 8 1a7 7 0 0 0 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/> <path fill-rule="evenodd" d="M10.97 4.97a.75.75 0 0 1 1.071 1.05l-3.992 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.236.236 0 0 1 .02-.022z"/> </svg>';
        if (comparison.vivienda_acv_imss_equality === false) {
          this.changeIcons(iconCompACV, 'icon-verify-ok', 'icon-verify-error', iconErr);
        }else this.changeIcons(iconCompACV, 'icon-verify-error', 'icon-verify-ok', iconOk);
        if (comparison.rcv_equality === false) {
          this.changeIcons(iconCompRCV, 'icon-verify-ok', 'icon-verify-error', iconErr);
        }else this.changeIcons(iconCompACV, 'icon-verify-error', 'icon-verify-ok', iconOk);
        if (comparison.total_equality === false) {
          this.changeIcons(iconCompTotal, 'icon-verify-ok', 'icon-verify-error', iconErr);
        }else this.changeIcons(iconCompACV, 'icon-verify-error', 'icon-verify-ok', iconOk);
    }
