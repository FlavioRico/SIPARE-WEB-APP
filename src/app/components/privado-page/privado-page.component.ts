        ...
        if(result.logged == 0)
          ...
          let dateNow = new Date();
            ...
            this.errorMsj = 'Rango de búsqueda por fecha debe ser hasta un día hábil anterior al día actual';
              ...
              this.processFile.getFilesRiceibingToProcesarByDateRange(this.dateRange.from, this.dateRange.to).subscribe(
              result => {           
                  if(result.resultCode == 0){
                      if (result.listSize == 0){
                        this.tableHidden = true;
						        ...
						        this.tableHidden = true;
						  		if(this.dateRange === undefined || this.dateRange === null){
									this.processFile.exportDataInFileXlsDefault().subscribe(
						  			...
						  			this.processFile.exportDataInFileXls(this.dateRange.from, this.dateRange.to).subscribe(
							  			result => { 
								  			if(result.byteLength > 0){
											this.isInfo= false;
									        this.infoCode = '';
